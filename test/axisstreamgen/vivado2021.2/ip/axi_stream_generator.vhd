...
architecture arch of axi_stream_generator is

   -- Total number of input data.
   constant NUMBER_OF_INPUT_WORDS  : natural := 32;
   ...
   signal tvalid_delay : std_logic := '0';
...
--   tvalid <= '1' when state = Write_Outputs else '0';
--   M_AXIS_TVALID <= tvalid_delay;

   M_AXIS_TDATA <= sum;
   M_AXIS_TLAST <= tlast; 
--   M_AXIS_TLAST <= tlast_delay;         
   state_check <= "00" when state = Idle else
                 "01" when state = Read_Inputs else
                 "10" when state = Write_Outputs else
                 "11";
                             
	process(ACLK)                                                                           
	begin                                                                                          
	  if (rising_edge (ACLK)) then    
	   if ((state = Write_Outputs)) then
	       S_AXIS_TREADY <= '0';
	   else
